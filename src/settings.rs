...
pub struct AppSettings {
    ...
    // [重构] Theme settings: 放弃固定的枚举，改用主题名称以支持无限的 JSON 自定义主题
...
impl Default for AppSettings {
    fn default() -> Self {
        Self {
            ...
            // 默认加载你专属的 STS Pro 预设主题
            theme_name: "STS Pro (Default)".to_string(),
...
impl AppSettings {
    ...
    pub fn load_from_registry() -> Self {
        ...
        if let Ok(hkcu) = RegKey::predef(HKEY_CURRENT_USER).open_subkey(REGISTRY_KEY) {
            ...
            // 读取主题名称
            if let Ok(theme_name) = hkcu.get_value::<String, _>("ThemeName") {
                settings.theme_name = theme_name;
            } else if let Ok(old_theme) = hkcu.get_value::<String, _>("ThemeMode") {
                // [向下兼容] 如果是旧版本升上来的，把旧的系统枚举映射到新的名字
                settings.theme_name = match old_theme.as_str() {
                    "dark" => "Classic Dark".to_string(),
                    "light" => "Classic Light".to_string(),
                    _ => "STS Pro (Default)".to_string(),
                };
            }
        }
    ...
    pub fn save_to_registry(&self) -> Result<(), String> {
        ...
        // 存入自定义主题名称
        key.set_value("ThemeName", &self.theme_name)
            .map_err(|e| format!("Failed to save ThemeName: {}", e))?;
